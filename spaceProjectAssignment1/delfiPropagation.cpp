...
static inline std::string getOutputPath( )
{
    ...
    return ( filePath_.substr( 0, filePath_.length( ) -
                             std::string( "delfiPropagation.cpp" ).length( ) ) ) + "PropagationResults";
}
...
int main( )
{
    ...
    for( unsigned int assignmentQuestion = 1; assignmentQuestion <= 3; assignmentQuestion++ )
    {
        ...
        Eigen::Vector6d delfiInitialCartesianState =  convertKeplerianToCartesianElements(
                    initialKeplerElements, bodyMap[ "Earth" ]->getGravityFieldModel( )->getGravitationalParameter( ) );
        if( centralBodies.at( 0 ) == "SSB" )
        {
            delfiInitialCartesianState += bodyMap[ "Earth" ]->getStateInBaseFrameFromEphemeris( simulationStartEpoch );
        }
        ...
        // Save barycentric Earth state (only for 1.1)
        if( assignmentQuestion == 1  )
        {
